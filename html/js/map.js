					...
					marker: {
						...
						set: function(){
							MARKER.each(function(){
								var t = $(this), position = t.attr('data-coords').split(',');
									x = parseInt(position[0]-17), y = parseInt(position[1]),
									css = {
										position: 'absolute',
										zIndex: '2',
										top: y,
						...
						open: function(){
							MARKER.on('click', function(){
								...
								$('.marker').css('z-index', '2').removeClass('active');
								t.css('z-index', '20').addClass('active');
								...
								if (marker.popup){
									...
									t.after(
										$('<div />').addClass(marker.popup_name+' '+id + ' flipInY animated').css(css).html(html).append(
											$('<a />').addClass('close')
										)
									);
									...
									if (x+pw/2 > I.w){ // I cut something out here to make SF work, not reeeeeely sure what it was :)
										x0 = x-pw+w;
									} else {
										x0 = x-(pw/2-w/2);
									}
						...
						close: function(){
							$('.smartgridMap').on('click','.close', function(){
...
$(function(){
	...
	$('.map-switcher').click(function(){
		$('.locations-list').fadeOut(100);
		$('.smartgridMap').fadeIn(500);	
		$(this).addClass("active");
		$('.list-switcher').removeClass("active");
	});

	$('.list-switcher').click(function(){
		$('.smartgridMap').fadeOut(100);
		$('.locations-list').fadeIn(500);
		$(this).addClass("active");
		$('.map-switcher').removeClass("active");
	});

	$('.smartgridMap').craftmap({
		fullscreen: true,
		marker:{center: true},
		image: {
			width: 3000,
